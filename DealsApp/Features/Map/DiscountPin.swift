    ...
    var body: some View {
        VStack(spacing: 0) {
            Text(verbatim: PriceFormatter().percent(pin.percent))
                ...
                .monospacedDigit()
                .foregroundColor(isSelected ? .white : Theme.dealAccentText)
                .padding(.horizontal, 8)
                .padding(.vertical, 5)
                .background(Capsule().fill(isSelected ? Theme.ink : Theme.dealAccent))
                .overlay(Capsule().stroke(Color.white, lineWidth: 1.5))
            Triangle()
                .fill(isSelected ? Theme.ink : Theme.dealAccent)
